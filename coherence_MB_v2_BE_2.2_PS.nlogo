...
;; Peter changed network update: link-creation only for receiver who just dropped a link (12/08/2022)
...
to go
  ...
  ask links [
    ...
    if prob prob_soc_infl [
      let receiver be-socially-influenced   ; process 1 (includes link health update)
      if prob (drop-bad-link * (logistic (link_health) (k_link))) [
        ask receiver[
          add-new-link ; process 3
        ]
        die ;; process 4 using same logistic function with (negative) k_link
      ]
    ]
  ]
  ...
  ask turtles [
    ...
    ;; REMOVED no random link creation without social influence! if prob prob_add_link [add-new-link] ; process 3
...
to-report be-socially-influenced
  let new_coherency 0
...
169
storing_folder
/home/peter/PhD_ZMT/Project_coherentagents/coherentagents/sim_data
1
0
String
