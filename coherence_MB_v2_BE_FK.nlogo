...
to initialize-globals
  ...
  ifelse file-exists? agent_data  [
    ...
    set item_labels but-first but-last but-last csv:from-row file-read-line
  ...
  ifelse file-exists? corol_mat_file [
    ...
    while [not file-at-end?] [
      ...
      let l []
      while [j <= num_items][  ;;
        let fl ((but-last (but-last (csv:from-row file-read-line))))   ;; also hardcoded: We know that we do not use for consistence matrix the first one and last two values
        let flp []
...
to initialize-agents
  ;; initialise agent beliefs
    ...
    (foreach (sort turtles) [ [t] ->
      ask t [
        set line but-last csv:from-row file-read-line
        ;; show line
